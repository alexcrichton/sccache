    ...
    where I: AsyncRead + AsyncWrite + 'static,
{
    ...
    fn bind_transport(&self, io: I) -> Self::BindTransport {
        future::ok(SccacheTransport {
            inner: WriteJson::new(ReadJson::new(Framed::new(io))),
        })
    }
}
...
/// * Next that sink/stream is wrapped in `ReadJson` which will cause the
///   `Stream` implementation to switch from `BytesMut` to `Request` by parsing
///   the bytes as json.
/// * Finally that sink/stream is wrapped in `WriteJson` which will cause the
...
struct SccacheTransport<I: AsyncRead + AsyncWrite> {
    inner: WriteJson<ReadJson<Framed<I>, Request>, Response>,
}
